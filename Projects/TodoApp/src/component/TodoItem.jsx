...
function TodoItem({item}) {
  ...
  return (
	<div display="flex justify-center content:center space-x-2">
		...
		<input className={`w-5/12 h-12 rounded-lg border-purple-800 drop-shadow-2xl text-center
			...
			readOnly={!editable}
		/>
		<button className="rounded-xl bg-purple-800 border-2 border-white text-white font-bold mx-6 p-4 shadow-2xl
          hover:border-2 hover:border-purple-800 hover:bg-white hover:text-purple-800 
		  " 
		  
		  onClick= {updateMsg}>{!editable? 'Edit':'save'}</button>
